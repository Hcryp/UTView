...
<div class="flex flex-col space-y-6 pb-12">
    ...
    <div x-data="{show: true}" x-show="show" class="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded shadow-sm flex justify-between items-center">
        <span class="font-medium flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> {{ session('success') }}</span>
        <button @click="show = false" class="text-green-500 hover:text-green-700">&times;</button>
    </div>
    @endif
    @if(session('error'))
    <div x-data="{show: true}" x-show="show" class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-sm flex justify-between items-center">
        <span class="font-medium flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ session('error') }}</span>
        ...
        <form id="filterForm" method="GET" action="{{ route('manpower.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            ...
            <div class="md:col-span-2">
                ...
                <select id="filterStatus" name="status" class="w-full text-sm border-gray-300 bg-slate-50 rounded focus:bg-white focus:ring-blue-500 cursor-pointer">
                    ...
                    <optgroup label="Inactive / Separation">
                        @foreach($out_reasons as $reason) <option value="{{ $reason }}" {{ request('status') == $reason ? 'selected' : '' }}>{{ $reason }}</option> @endforeach
                    </optgroup>
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <select id="filterDept" name="department" class="w-full text-sm border-gray-300 bg-slate-50 rounded focus:bg-white focus:ring-blue-500 cursor-pointer">
                    ...
                    @foreach($departments as $dept) <option value="{{ $dept }}" {{ request('department') == $dept ? 'selected' : '' }}>{{ $dept }}</option> @endforeach
                </select>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
        <h3 class="text-xs font-bold text-slate-500 uppercase mb-3">Recent Recaps</h3>
        <div class="flex gap-4 overflow-x-auto pb-2">
            @foreach($logs as $log)
            <a href="{{ route('manpower.log.show', $log->id) }}" class="min-w-[200px] p-3 border border-slate-200 rounded bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300 transition-all flex flex-col group cursor-pointer relative">
                <div class="flex justify-between items-center mb-2">
                    <span class="text-xs font-bold text-slate-700 group-hover:text-indigo-800">{{ $log->log_date->format('d M Y') }}</span>
                    <span class="text-[10px] text-slate-400">{{ $log->updated_at->format('H:i') }}</span>
                </div>
                <div class="text-xs text-slate-600 group-hover:text-slate-800">
                    <div class="flex justify-between"><span>MP:</span> <span class="font-mono font-bold">{{ $log->total_mp }}</span></div>
                    <div class="flex justify-between"><span>Hours:</span> <span class="font-mono font-bold">{{ number_format($log->total_mh,1) }}</span></div>
                </div>
                <div class="absolute inset-0 border-2 border-indigo-400 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity"></div>
            </a>
            @endforeach
        </div>
    </div>
            ...
            <table class="w-full text-sm text-left whitespace-nowrap">
                <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200 sticky top-0 z-10 font-bold select-none">
                    <tr>
                        ...
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100 group" onclick="toggleSort('name')">
                            <div class="flex items-center gap-1">Name / NRP @if(request('sort','name')=='name')<svg class="w-3 h-3 {{request('dir')=='desc'?'rotate-180':''}}" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>@endif</div>
                        </th>
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100 group" onclick="toggleSort('company')">
                            <div class="flex items-center gap-1">Company / Site @if(request('sort')=='company')<svg class="w-3 h-3 {{request('dir')=='desc'?'rotate-180':''}}" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>@endif</div>
                        </th>
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100 group" onclick="toggleSort('role')">
                            <div class="flex items-center gap-1">Role / Dept @if(request('sort')=='role')<svg class="w-3 h-3 {{request('dir')=='desc'?'rotate-180':''}}" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>@endif</div>
                        </th>
                        <th class="px-6 py-4 cursor-pointer hover:bg-slate-100 group" onclick="toggleSort('contract')">
                            <div class="flex items-center gap-1">Contract @if(request('sort')=='contract')<svg class="w-3 h-3 {{request('dir')=='desc'?'rotate-180':''}}" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>@endif</div>
                        </th>
                        <th class="px-6 py-4 text-right cursor-pointer hover:bg-slate-100" onclick="toggleSort('manhours')">
                            <div class="flex items-center justify-end gap-1">Manhours @if(request('sort')=='manhours')<svg class="w-3 h-3 {{request('dir')=='desc'?'rotate-180':''}}" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>@endif</div>
                        </th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php $isInactive = $row->status === 'INACTIVE'; $isExpired = !$isInactive && $row->end_date && strtotime($row->end_date) < time(); @endphp
                    <tr class="group hover:bg-blue-50/50 transition-colors {{ $isInactive ? 'bg-slate-50 opacity-75 grayscale-[0.5]' : ($isExpired ? 'bg-red-50/40' : '') }}">
                        ...
                        <td class="px-6 py-3 text-center">
                            <div class="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('manpower.show', $row->id) }}" class="p-1.5 text-blue-600 bg-blue-100 hover:bg-blue-200 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></a>
                                <a href="{{ route('manpower.edit', $row->id) }}" class="p-1.5 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-3"><div class="font-semibold text-slate-700">{{ Str::limit($row->company, 25) }}</div><div class="text-xs text-slate-500">{{ $row->site }}</div></td>
                        ...
                        <td class="px-6 py-3 text-xs">
                            ...
                            @if($row->end_date) @if($isExpired) <div class="text-red-700 font-bold mt-0.5 flex items-center gap-1"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Expired: {{ date('d M Y', strtotime($row->end_date)) }}</div>
                            @else <div class="text-slate-600 mt-0.5">End: {{ date('d M Y', strtotime($row->end_date)) }}</div> @endif
                            @else <div class="text-emerald-600 font-bold mt-0.5">Permanent</div> @endif
                        </td>
                        <td class="px-6 py-3 text-right font-mono font-bold text-slate-700">{{ number_format($row->manhours, 1) }}</td>
                        <td class="px-6 py-3 text-center">
                            @if($isInactive) <div class="flex flex-col items-center"><span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-slate-200 text-slate-600 border border-slate-300">{{ $row->out_reason ?? 'INACTIVE' }}</span>@if($row->date_out)<span class="text-[10px] text-slate-500 mt-0.5">{{ date('d/m/y', strtotime($row->date_out)) }}</span>@endif</div>
                            @elseif($isExpired) <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-800 border border-red-200 animate-pulse">EXPIRED</span>
                            @else <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-green-100 text-green-800 border border-green-200"><span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1.5"></span> ACTIVE</span> @endif
                        </td>
                    </tr>
                    @empty <tr><td colspan="8" class="px-6 py-24 text-center text-slate-400 font-medium">No personnel found matching criteria.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="bg-slate-50 border-t border-slate-200 px-6 py-3 flex justify-between items-center"><div class="text-xs text-slate-500">Showing {{ $data->firstItem() }}-{{ $data->lastItem() }} of <strong>{{ $data->total() }}</strong></div><div class="scale-90">{{ $data->appends(request()->query())->links() }}</div></div>
        @endfragment
    </div>
</div>
...
<div x-data="{ open: false }" class="fixed bottom-0 right-8 z-40 print:hidden">
    <button @click="open = !open" class="bg-slate-800 hover:bg-slate-700 text-white px-5 py-3 rounded-t-lg shadow-lg flex items-center gap-3 transition-colors text-xs font-bold uppercase tracking-wider border border-slate-700">
        <svg class="w-4 h-4 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg> Detailed Report <svg :class="{'rotate-180': open}" class="w-4 h-4 transition-transform duration-300 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
    </button>
    <div x-show="open" x-cloak x-transition.origin.bottom class="w-[600px] h-[500px] bg-white shadow-2xl border border-slate-200 rounded-tl-lg flex flex-col overflow-hidden">
        ...
        <div class="overflow-y-auto p-4 space-y-6 scrollbar-thin bg-slate-50 h-full">
            <table class="w-full text-xs bg-white shadow-sm rounded overflow-hidden"><thead class="bg-blue-50 text-blue-800 border-b border-blue-100"><tr><th class="p-3 text-left">Category</th><th class="p-3 text-right">MP</th><th class="p-3 text-right">Hours</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach($highLevelTable as $row)<tr><td class="p-3 font-bold">{{ $row->category }}</td><td class="p-3 text-right">{{ $row->mp }}</td><td class="p-3 text-right font-mono">{{ number_format($row->mh) }}</td></tr>@endforeach</tbody></table>
            <table class="w-full text-xs bg-white shadow-sm rounded overflow-hidden"><thead class="bg-yellow-50 text-yellow-800 border-b border-yellow-100"><tr><th class="p-3 text-left">Company Detail</th><th class="p-3 text-right">MP</th><th class="p-3 text-right">Hours</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach($detailedTable as $row)<tr><td class="p-3"><span class="block font-bold text-slate-700">{{ $row->company }}</span><span class="text-[10px] text-slate-400">{{ $row->category }}</span></td><td class="p-3 text-right">{{ $row->mp }}</td><td class="p-3 text-right font-mono">{{ number_format($row->mh) }}</td></tr>@endforeach</tbody></table>
        </div>
    </div>
</div>
